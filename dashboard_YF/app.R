...
df$major_genre<-unlist(lapply(df$genres, function(x) x[[1]][1]))
...
ui<-navbarPage("  Movie Facts",  tags$style(HTML(".navbar .navbar-default .navbar-static-top {margin-bottom:0px;padding-bottom:0px;}")),
      ...
      ")),
                            sliderInput("time",
                                        label =tags$span("Range of run time (minutes):",style = "padding-left:0px;font-weight: bold;font-size:13px;margin-bottom:0px;padding-bottom:0px"),
                                        min = 0,
                                        max = 240,
                                        ticks=FALSE,
                                        value=c(0,240),
      ...
      ")),
                            ...
                            radioButtons(inputId = "genre", label = "",
                                         choices = list("Art House & International"="Art House & International","Animation" = "Animation","Kids & Family"="Kids & Family","Drama" = "Drama", 
                                                        "All"="All Movies","Mystery & Suspense"="Mystery & Suspense" ,"Comedy" = "Comedy","Science Fiction & Fantasy"="Science Fiction & Fantasy",
                                                        "Horror"="Horror","Action & Adventure" = "Action & Adventure","Romance"="Romance"),
                                         selected = "All Movies"),
                            hr(),
                          ...
                          dashboardBody(
                            ...
                            fluidRow(
                              ...
                              box(width = 4,
                                  ...
                                  tags$h4("Visulizations answer such questions strikingly.",style = "padding-left:3px;margin-bottom:0px;font-family: Century Gothic, fantasy;")),
...
server<-function(input, output,session) {
  ...
  output$ratio <- renderValueBox({
    valueBox(
      value = tags$p(scales::percent((mean(data()$crt_prf,na.rm=TRUE))),style="font-size:38px;font-family: Century Gothic, fantasy;"),
  ...
  output$plot <- renderPlotly({
    if(nrow(data())==0){
      ...
      ggplotly(data()%>%ggplot()+
                 geom_polygon(data=tri,mapping=aes(x=x,y=y),color=NA,fill="grey",alpha=0.2)+
                 geom_point(aes(audience_rating,tomatometer_rating,size=revenue,color=major_genre,text=title),alpha=0.4)+
                 theme_minimal()+
                 scale_x_continuous(breaks=seq(25,100,25),limits = c(0,100),expand = c(0,0)) +
    ...
    else if((input$genre=="All Movies")&(length(input$genres)==0)){
      ggplotly(data()%>%ggplot()+
                 geom_polygon(data=tri,mapping=aes(x=x,y=y),color=NA,fill="grey",alpha=0.2)+
                 geom_point(aes(audience_rating,tomatometer_rating,size=revenue,color=major_genre,text=title),alpha=0.4)+
                 theme_minimal()+
      ...
      ggplotly(data()%>%ggplot()+
                 geom_polygon(data=tri,mapping=aes(x=x,y=y),color=NA,fill="grey",alpha=0.2)+
                 geom_point(aes(audience_rating,tomatometer_rating,size=revenue,color=major_genre,text=paste("movie title:",title)),alpha=0.4)+
                 scale_x_continuous(breaks=seq(25,100,25),limits = c(0,100),expand = c(0,0))+
                 scale_y_continuous(limits = c(0,100),expand = c(0,0)) +
                 geom_abline(intercept=alpha,slope=beta,color="grey47",alpha=0.6,size=2)+
